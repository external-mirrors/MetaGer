...
class SuggestionDebtAuthorization extends Authorization
{
    ...
    {
        $this->availableTokens = max(self::GET_CREDIT() - self::GET_DEBT(), 0);
    }
    ...
    public static function ADD_CREDIT(float $amount = 0.1)
    {
        ...
        Redis::connection(config('cache.stores.redis.connection'))::hexpireat($cache_key, "credit", $expiration->getTimestamp());
    }
    ...
    public static function ADD_DEBT(float $amount = 0.1)
    {
        ...
        Redis::connection(config('cache.stores.redis.connection'))::hexpireat($cache_key, "debt", $expiration->getTimestamp());
    }
    ...
    public static function UPDATE_SETTINGS(bool $reenable = false)
    {
        ...
        if ($stored_settings !== null) {
            ...
            if (in_array($settings->suggestion_provider, [null, "off"])) {
                ...
                $stored_settings = [
                    ...
                    "delay" => $settings->suggestion_delay
                ];
            } else {
        ...
        if ($stored_settings !== null) {
            ...
            Redis::connection(config('cache.stores.redis.connection'))::hexpireat($cache_key, "settings", $expiration->getTimestamp());
        }
    }
